...
std::string backendSharedLibDir = BACKEND_SHARED_LIB_DIR;
...
{
    STT<WhisperImpl> stt;
    auto* context = stt.InitContext<whisper_context>(modelPath.c_str(), backendSharedLibDir.c_str());
    std::vector<float> audioData = ReadAudioData(audioDataPath);

    constexpr bool printRealtime = true;
    constexpr bool printProgress = false;
    constexpr bool timeStamps = true;
    constexpr bool printSpecial = false;
    constexpr bool translate = false;
    const char *language = "en";
    constexpr int numThreads = 2;
    constexpr int offsetMs = 0;
    constexpr bool noContext = true;
    constexpr bool singleSegment = false;
